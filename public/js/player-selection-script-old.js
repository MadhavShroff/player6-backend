...
$(document).ready(() => {
    ...
    MemberStack.onReady.then(async function(member) {
        ...
        socket.on('player update', data => {
            ...
            if(!(data.userID === uid) || oppSelectionCompleted()) { // if not this user
...
function myTurn(isWinner, sel) { 
    ...
    if(oppSelectionCompleted()) return true;
    if(sum % 2 === 0) {
...
function oppSelectionCompleted() {
    if(firstOrSecond === 1) {
        var u2count = 0; var missedCountUser2 = 0;
        $.each(currentSelection.user2, (idx, name) => {
            if(name !== "- Turn Missed -") u2count = u2count + 1;
            else missedCountUser2 = missedCountUser2 + 1;
        });
        if (u2count >= 6) return true;
        ...
        var u1count = 0; var missedCountUser1 = 0;
        $.each(currentSelection.user1, (idx, name) => {
            if(name !== "- Turn Missed -") u1count = u1count + 1;
            else missedCountUser1 = missedCountUser1 + 1;
        });
        if (u2count >= 6) return true;
