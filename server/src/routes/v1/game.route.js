...
// Game logic route
let globalClient;
MongoClient.connect(config.mongoose.url, {
	...
	db.collection('pendingGames')
		...
		.toArray((err, res) => {
			if (res.length === 0) {
				...
				db.collection('pendingGames').deleteOne({
					_id: res[0]._id
				});
				const newGame = {
					...res[0],
...
router.post('/getGame', (req, res) => {
	const db = globalClient.db('games');
	db.collection('matchedGames').findOne({
		pendingGameID: req.body.pendingID
	}, (err, result) => {
		let game = result;
		if (result == null) {
			res.send({
...
router.post('/updateTossWinner', (req, res) => {
	...
	db.collection('matchedGames')
		.updateOne({
			pendingGameID: req.body.gameID
		}, {
			$set: {
				tossWinner: req.body.winner
			}
		});
})
